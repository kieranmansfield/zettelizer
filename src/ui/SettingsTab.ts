...
export class ZettelizerSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		containerEl.createEl("h2", { text: "Zettelizer Settings" });
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Format for zettel filenames. Use: YYYY (year), MM (month), DD (day), HH (hour), mm (minute), ss (second), SSS (millisecond)"
			)
			.addText((text) =>
				text
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Maximum characters to display for each highlight in the selection modal"
			)
			.addText((text) =>
				text
					.setPlaceholder("100")
		...
		// Example template
		containerEl.createEl("h4", { text: "Example Template" });
