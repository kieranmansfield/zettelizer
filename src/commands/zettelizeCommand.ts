...
class ReadwiseFileModal extends FuzzySuggestModal<TFile> {
	...
	onChooseItem(file: TFile): void {
		zettelizeFile(this.plugin, file);
	}
}
...
async function zettelizeFile(plugin: ZettelizerPlugin, file: TFile) {
	...
	new FuzzyHighlightModal(
		...
		async (selectedHighlights) => {
			await createZettels(plugin, file, selectedHighlights);
		}
	).open();
}

async function createZettels(
	plugin: ZettelizerPlugin,
	sourceFile: TFile,
	...
	if (plugin.settings.templatePath) {
		...
		if (templateFile instanceof TFile) {
			...
			new Notice(
				`Template file not found: ${plugin.settings.templatePath}`
			);
		}
	}
	...
	for (const highlight of highlights) {
		...
		if (templateContent) {
			...
			if (!content.trim()) {
				new Notice(
					`Warning: Template produced empty content. Check template variables.`
				);
				// Fallback to default
	...
	new Notice(
		`Created ${created} zettel(s). Skipped ${skipped} (already exist).`
	);
}
